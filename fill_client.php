...
function fillClient(){
	...
	$query = "SELECT DISTINCT Campaign FROM `Data` WHERE Client = '".$_GET['Client']."'";
	$campaigns = mysql_query($query);

	$dataString= "";
	$campaignsString = "";
	while($campaign = mysql_fetch_array($campaigns)){
		$dataString .= "['<div class=\'campaign\' client=\'".$_GET['Client']."\' campaign=\'".utf8_encode($campaign['Campaign'])."\'>".utf8_encode($campaign['Campaign'])." </div><div class=\'adData\'>+</div>',";
		$qsArray = array();

		$query = "SELECT DISTINCT Year, Month FROM `Data` WHERE Client='".$_GET['Client']."' AND Campaign = '".$campaign['Campaign']."' ORDER BY Year, Month ASC";
		...
		while($period = mysql_fetch_array($periods)){
			...
			$query = "SELECT * FROM `Data` WHERE Client='".$_GET['Client']."' AND Campaign='".$campaign['Campaign']."' AND Year='".$period['Year']."' AND Month='".$period['Month']."'";
