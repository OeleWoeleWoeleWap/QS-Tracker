...
function fillCampaign(){
	...
	$query = "SELECT DISTINCT Adgroup FROM `Data` WHERE Client = '".$_GET['Client']."' AND Campaign = '".$_GET['campaign']."'";
	$adgroups = mysql_query($query);


	$dataString= "";
	$campaignsString = "";
	while($adgroup = mysql_fetch_array($adgroups)){
		

		$dataString .= "['<div class=\'adgroup\' client=\'".$_GET['Client']."\' campaign=\'".$_GET['campaign']."\' adgroup=\'".utf8_encode($adgroup['Adgroup'])."\'>".utf8_encode($adgroup['Adgroup'])." </div><div class=\'adData\'>+</div>',";
		$qsArray = array();

		$query = "SELECT DISTINCT Year, Month FROM `Data` WHERE Client='".$_GET['Client']."' AND Campaign = '".$_GET['campaign']."' AND Adgroup = '".$adgroup['Adgroup']."' ORDER BY Year, Month ASC";
		...
		while($period = mysql_fetch_array($periods)){
			...
			$query = "SELECT * FROM `Data` WHERE Client='".$_GET['Client']."' AND Campaign='".$_GET['campaign']."' AND Adgroup = '".$adgroup['Adgroup']."' AND Year='".$period['Year']."' AND Month='".$period['Month']."'";
